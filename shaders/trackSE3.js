const se3TrackSource = `#version 310 es
layout(local_size_x = 32, local_size_y = 32) in;

layout(binding = 0, rgba8) readonly uniform highp image2D lastColorMap;
layout(binding = 1, rgba8) readonly uniform highp image2D nextColorMap;

layout(binding = 2, rgba32f) readonly uniform highp image2D gradientMap;
...
layout(binding = 5, r32f) readonly uniform highp image2D lastDepthMap;
layout(binding = 6, r32f) readonly uniform highp image2D nextDepthMap;

//layout(binding = 7, rgba32f) readonly uniform highp image2D outputTestImage;
...
void computeResiduals()
{
	...
	if (nextColorLoc.x >= 0 && nextColorLoc.y >= 0 && nextColorLoc.x < imSize.x && nextColorLoc.y < imSize.y)
	{
		...
		for (int u = nextColorLoc.x - 2; u < nextColorLoc.x + 2; u++)
		{
			...
			{
				valid = valid && (luminance(imageLoad(nextColorMap, ivec2(u,v)).xyz) > 0.0f);
			}
		}
		...
		if (valid)
		{
			...
			if (mTwo >= 0.00025f)
			{
				...
				if (!isnan(d1) || d1 != 0.0f)
				{
					...
					if (lastDepthLoc.x >= 0.0f && lastDepthLoc.y >= 0.0f && lastDepthLoc.x < float(imSize.x) && lastDepthLoc.y < float(imSize.y))
					{
						...
						if (d0 != 0.0f && !isnan(d0))
						{
							...
							if (d0 > 0.0f && abs(transformed_d1 - d0) <= maxDepthDelta && lastColor != 0.0f)
							{
