const se3TrackStepSource = `#version 310 es
...
void main()
{
	...
	if (found_corresp)
	{
		...
		if (cloudPoint.z == 0.0f || isnan(cloudPoint.z))
		{
			row.data[0] = row.data[1] = row.data[2] = row.data[3] = row.data[4] = row.data[5] = row.data[6] = row.inliers = 0.0f;
		}
		else
		{
